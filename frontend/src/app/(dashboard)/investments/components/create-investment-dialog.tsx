...
type FormData = z.infer<typeof createInvestmentSchema>;
...
const CreateInvestmentDialog = () => {
    ...
    const form = useForm<FormData>({
        ...
        defaultValues: { date: new Date().toISOString().split("T")[0], lender: "", amountReceived: 0, mode: "Cash", referenceNo: "", notes: "" },
    });

    const createRequest = async (values: FormData) => {
        setToastId(toast.loading("Recording…", { description: "Saving investment record." }));
        return await axios.post("/investments", values);
    };
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent onOpenAutoFocus={e => e.preventDefault()} className="overflow-hidden p-0 sm:max-w-lg max-h-[90vh] flex flex-col bg-card border-border">
                <div className="glass-header relative p-4 sm:p-6">
                    ...
                    <div className="relative flex items-center gap-3">
                        ...
                        <div className="min-w-0 flex-1">
                            ...
                            <DialogTitle className="m-0 text-lg font-bold text-foreground sm:text-2xl">Record Investment</DialogTitle>
                            <DialogDescription className="mt-0.5 hidden text-xs text-muted-foreground sm:mt-1 sm:block sm:text-sm">
                                Enter the capital details received from the lender
                            </DialogDescription>
                        </div>
                    </div>
                </div>
                ...
                <Form {...form}>
                    <form id="create-investment-form" onSubmit={form.handleSubmit(values => mutate(values))} className="flex flex-col flex-1 overflow-hidden min-h-0">
