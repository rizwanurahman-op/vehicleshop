...
interface InvestmentExportFilters {
    mode?: string;
    dateFrom?: string;
    dateTo?: string;
    search?: string;
...
export const exportInvestmentsPDF = async (investments: InvestmentRow[], filters: InvestmentExportFilters = {}): Promise<Buffer> => {
    const total = investments.reduce((s, i) => s + (i.amountReceived ?? 0), 0);
    const modeMap = new Map<string, number>();
    investments.forEach(i => modeMap.set(i.mode, (modeMap.get(i.mode) ?? 0) + i.amountReceived));
    ...
    return new Promise((resolve, reject) => {
        ...
        const C = {
            ...
            cyan:   "#0891b2", violet: "#7c3aed", slate:  "#64748b",
        ...
        doc.rect(0, 50, PW, 4).fill(C.violet);
        ...
        const titleParts = ["Investments Report"];
        ...
        doc.fontSize(7.5).font("Helvetica").fillColor(C.muted)
            .text(`${genStr}  |  ${investments.length} records`, MG, 31, { width: CW, align: "right", lineBreak: false });
        ...
        const uniqueLenders = new Set(investments.map(i => typeof i.lender === "object" ? (i.lender as {name?:string})?.name : i.lender)).size;
        const avgAmt = investments.length > 0 ? Math.round(total / investments.length) : 0;

        const metrics = [
            { label: "TOTAL INVESTMENTS", value: String(investments.length), sub: `From ${uniqueLenders} lenders`,          accent: C.indigo },
            { label: "TOTAL RECEIVED",    value: dINR(total),                sub: "Cumulative capital received",             accent: C.violet },
            { label: "AVERAGE AMOUNT",    value: dINR(avgAmt),               sub: "Per investment",                         accent: C.cyan   },
            { label: "TOP PAYMENT MODES", value: topModes[0]?.[0] || "-",   sub: modeSub,                                   accent: C.green  },
        ];
        ...
        // ── TABLE ──────────────────────────────────────────────────────────
        // Cols: 16+64+52+90+90+78+80+100+96 = 666 (CW ~786 — fine)
        ...
        if (investments.length === 0) {
            ...
            doc.fontSize(8).font("Helvetica").fillColor(C.muted).text("No investments match the selected filters.", MG + 12, y + 10, { lineBreak: false });
            y += 28;
        } else {
            investments.forEach((inv, idx) => {
                ...
                const textY = y + 4;
                const lenderObj = typeof inv.lender === "object" ? inv.lender as { lenderId?: string; name?: string } : null;
                let rx = MG;
                ...
                cell(inv.investmentId ?? "-",                    1, C.indigo);
                cell(dFmt(inv.date),                             2, C.muted);
                cell(trunc(lenderObj?.name ?? "-", 18),          3, C.text, true);
                cell(lenderObj?.lenderId ?? "-",                 4, C.slate);
                cell(dINR(inv.amountReceived),                   5, C.violet, true);
                cell(inv.mode || "-",                            6, C.cyan);
                cell(trunc(inv.referenceNo, 16),                 7, C.slate);
                cell(trunc(inv.notes, 18),                       8, C.slate);
        ...
        doc.fontSize(6.5).font("Helvetica-Bold").fillColor(C.white)
            .text(`${investments.length} investments  |  Total: ${dINR(total)}  |  Avg: ${dINR(avgAmt)}  |  ${modeSummary}`,
