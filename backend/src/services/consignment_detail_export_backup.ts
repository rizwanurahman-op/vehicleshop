...
const dINR = (n: number | null | undefined) =>
    n == null ? "â€”" : `Rs. ${Math.abs(n).toLocaleString("en-IN")}`;
...
const dSl = (s: string | null | undefined) =>
    s ? s.replace(/_/g, " ").replace(/\b\w/g, (c) => c.toUpperCase()) : "â€”";
        ...
        try {
            ...
            const addFooter = () => {
                ...
                doc.fontSize(6.5).font("Helvetica").fillColor(C.muted)
                   .text("VehicleBook â€” Confidential. For internal use only.", MG, PH - 14, { lineBreak: false });
            };
            ...
            doc.fontSize(8).font("Helvetica").fillColor(C.muted)
               .text(`Generated: ${new Date().toLocaleString("en-IN", { day: "2-digit", month: "short", year: "numeric", hour: "2-digit", minute: "2-digit" })}`,
            ...
            doc.fontSize(7.5).font("Helvetica-Bold").fillColor(accentColor)
               .text(`${v.consignmentId}  Â·  ${isParkSale ? "Park Sale" : "Finance Sale"}`, MG + 12, y + 8, { lineBreak: false });
            ...
            doc.fontSize(8.5).font("Helvetica").fillColor(C.slate)
               .text(`${v.registrationNo}  Â·  ${v.vehicleType === "two_wheeler" ? "Two Wheeler" : "Four Wheeler"}`, MG + 12, y + 40, { lineBreak: false });
            ...
            doc.fontSize(7).font("Helvetica").fillColor(C.muted)
               .text(`Owner: ${v.previousOwner}  Â·  Received: ${dFmt(v.dateReceived)}`, MG + 12, y + 52, { lineBreak: false });
            y += 70;
            ...
            const vRows: [string, string, boolean?][] = [
                ["Make", v.make || "â€”", true],
                ["Model", v.model || "â€”", true],
                ["Year", v.year?.toString() ?? "â€”"],
                ["Registration No", v.registrationNo || "â€”", true],
                ["Color", (v as any).color ?? "â€”"],
                ["Engine No", (v as any).engineNo ?? "â€”"],
                ["Chassis No", (v as any).chassisNo ?? "â€”"],
            ...
            vRows.forEach(([l, val, bold], i) => {
                ...
                doc.font(bold ? "Helvetica-Bold" : "Helvetica").fillColor(C.text)
                   .text(String(val), MG + colW / 2, leftY + 5, { width: colW / 2 - 6, align: "right", lineBreak: false });
                leftY += 17;
            });
            ...
            const oRows: [string, string, boolean?][] = [
                [isParkSale ? "Owner" : "Finance Owner", v.previousOwner || "â€”", true],
                ["Phone", (v as any).previousOwnerPhone ?? "â€”"],
            ...
            oRows.forEach(([l, val, bold], i) => {
                ...
                doc.font(bold ? "Helvetica-Bold" : "Helvetica").fillColor(C.text)
                   .text(String(val), rx + colW / 2, rightY + 5, { width: colW / 2 - 6, align: "right", lineBreak: false });
                rightY += 17;
            });
                    ...
                    if (bd?.items?.length) {
                        bd.items.forEach((item: any) => {
                            ...
                            doc.fontSize(6.5).font("Helvetica").fillColor(C.slate)
                               .text((item.name ?? "") + (item.date ? "  Â·  " + dFmt(item.date) : ""), MG + 14, y + 4, { width: CW - 90, lineBreak: false });
            ...
            if (isSold) {
                ...
                const saleRows: [string, string, string?, boolean?][] = [
                    ...
                    ["Sold To",               (v as any).soldTo ?? "â€”",          undefined, true],
                    ["Buyer Phone",            (v as any).soldToPhone ?? "â€”"],
                    ...
                    ["Days in Shop",           v.daysInShop != null ? `${v.daysInShop} days` : "â€”"],
            ...
            if (buyerPayments.length > 0) {
                ...
                buyerPayments.forEach((p: any, i: number) => {
                    ...
                    doc.font("Helvetica").fillColor(C.text)
                       .text(dFmt(p.date), MG + 26, y + 6, { lineBreak: false });
                    const modeLabel = p.type === "exchange" ? `Exchange (${p.exchangeVehicleMake ?? ""})` : (p.mode ?? "");
                    doc.fillColor(p.type === "exchange" ? C.amber : C.muted)
                       .text(`via ${modeLabel}${p.notes ? " â€” " + p.notes : ""}`, MG + 115, y + 6, { width: 200, lineBreak: false });
            ...
            if (payeePayments.length > 0) {
                ...
                y = sectionBar(
                    `PAYEE PAYMENTS â€” ${isParkSale ? "Owner" : "Finance"} (Money Out)`,
                ...
                payeePayments.forEach((p: any, i: number) => {
                    ...
                    doc.fillColor(C.muted)
                       .text(`via ${p.mode ?? ""}${p.notes ? " â€” " + p.notes : ""}`, MG + 115, y + 6, { width: 200, lineBreak: false });
